    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
            {
                ...
                bendDir.xz *= 0.4; //make grass shorter when bending, looks better
                bendDir.y = min(-0.1,bendDir.y);//prevent grass become too long if camera forward is / near parallel to ground
                positionOS = lerp(positionOS.xyz + bendDir * positionOS.y / -bendDir.y, positionOS.xyz, stepped * 0.80 + 0.05);//don't fully bend, will produce ZFighting
