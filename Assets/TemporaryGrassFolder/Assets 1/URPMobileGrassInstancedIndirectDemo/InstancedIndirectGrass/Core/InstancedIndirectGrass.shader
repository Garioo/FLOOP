    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
            {
                ...
                bendDir.xz *= 1; //make grass shorter when bending, looks better
                bendDir.y = min(3,bendDir.y);//prevent grass become too long if camera forward is / near parallel to ground
                positionOS = lerp(positionOS.xyz + bendDir * positionOS.y / -bendDir.y, positionOS.xyz, stepped * 1 + 0.05);//don't fully bend, will produce ZFighting
